...
void md5(uint8_t *initial_msg, size_t initial_len, uint8_t *hash)
{
    ...
    uint32_t h0 = 0x67452301;//高位儲存 (0x01 0x23 0x45 0x67)
    ...
    msg[initial_len] = 128;//1000_0000 (一個1後面接0
    ...
    for (offset = 0; offset < pad_len; offset += (512/8))
    {
		...
		for(i = 0; i < 64; i++)
        {
            ...
            if(i<16){
//                printf("%d a:%u f:%u k[i]:%u w[g]:%u\n",i,a,f,k[i],w[g]);
            }
//            printf("%d a:%u b:%u c:%u d:%u tot:%u\n",i,a,b,c,d,a+f+k[i]+w[g]);
            uint32_t temp = d;
            d = c;
            c = b;
...
int main(int argc, char **argv)
{
    ...
    printf("%02x%02x%02x%02x\n", hash[12], hash[13], hash[14], hash[15]); //HEX兩位輸出hash[i] 不足補0
    return 0;
}
//25d55ad283aa400af464c76d713c07ad 12345678
//dd4b21e9ef71e1291183a46b913ae6f2 00000000
//e8cd0953abdfde433dfec7faa70df7f6 53589793